...
logic r_o_stall;
logic init_val;
logic [31:0] i_fifoempty_ext;
assign i_fifoempty_ext = {{32-EMPTY_PORT_WIDTH{1'b0}},i_fifoempty};
...
init_reg
  #( .WIDTH    ( DATA_WIDTH + 2 + 32 ),
     .INIT     ( INIT     ),
     .INIT_VAL ( {32'b0, 2'b00, INIT_VAL} ),
     .ASYNC_RESET(ASYNC_RESET),
...
reg_data ( 
      ...
      .reset   ( init_reset ),
      .i_init  ( init_val   ),
      .i_data  ( {i_fifoempty_ext, i_fifostartofpacket, i_fifoendofpacket, i_fifodata} ),
      .i_valid ( i_fifovalid ), 
      .o_valid ( initvalid ),
      .o_data  ( {o_packetempty, o_startofpacket, o_endofpacket, o_data} ),
      .o_stall ( r_o_stall ),
      .i_stall ( ~initready ) 
      );
end
...
assign o_packetempty = i_fifoempty_ext;
end
endgenerate
